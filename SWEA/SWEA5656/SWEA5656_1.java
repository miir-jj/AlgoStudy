...
/**
 * @author 미령 5656 벽돌깨기 구슬위치 -> 중복순열 구슬이 떨어질 수 있는 가장 윗행의 벽돌 찾기
 */
public class SWEA5656_1 {
	...
	private static void shoot() {
		...
		for (int n = 0; n < N; n++) {
			...
			q.offer(new int[] { r, marble[n], tmap[r][marble[n]] });
			tmap[r][marble[n]] = 0;
			while (!q.isEmpty()) {
				int[] cur = q.poll();
				int nr, nc;
				for (int d = 0; d < 4; d++) {
					for (int l = 1; l < cur[2]; l++) {
						nr = cur[0] + dr[d] * l;
						nc = cur[1] + dc[d] * l;
						if (nr >= 0 && nr < H && nc >= 0 && nc < W) {
							if (tmap[nr][nc] > 0) {
								q.offer(new int[] { nr, nc, tmap[nr][nc] });
							}
							tmap[nr][nc] = 0;
						}
					}
